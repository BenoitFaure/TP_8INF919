...
# Get memory limit on gpu
memory_limit = 2048 # MB
mem_limit_per_partition = int(memory_limit / num_partitions)

mem_limit_per_partition = 512 # MB

# Set memory limit on gpu
tf.config.experimental.set_virtual_device_configuration(gpus[0], [
    tf.config.experimental.VirtualDeviceConfiguration(memory_limit=mem_limit_per_partition) 
...
def train_test_model(model):
    ...
    # Callbacks
    name_str = datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + '_' + model_name + '_' + str(num_partitions)
